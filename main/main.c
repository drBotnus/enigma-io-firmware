...
#define GPIO_INPUT_PIN_MASK \
	((1ULL << GPIO_SWITCH_INPUT_PIN_0) | (1ULL << GPIO_SWITCH_INPUT_PIN_1))
/*
...
#define GPIO_OUTPUT_PIN_MASK \
	((1ULL << GPIO_LED_OUTPUT_PIN_0) | (1ULL << GPIO_LED_OUTPUT_PIN_1))
/*
...
static void gpio_button_task(void *args)
{
	...
	while (1) {
		if (xQueueReceive(gpio_button_evt_queue, &gpio_num,
				  portMAX_DELAY)) {
			uint32_t *led_state =
				(gpio_num == GPIO_SWITCH_INPUT_PIN_0) ?
					&led_state_0 :
					&led_state_1;
			*led_state = !*led_state;
			...
			uint32_t led_pin =
				(gpio_num == GPIO_SWITCH_INPUT_PIN_0) ?
					GPIO_LED_OUTPUT_PIN_0 :
					GPIO_LED_OUTPUT_PIN_1;
...
void app_main(void)
{
	...
	debounce_timer_0 = xTimerCreate(
		"debounce_timer_0", pdMS_TO_TICKS(DEBOUNCE_DELAY_MS), pdFALSE,
		(void *)GPIO_SWITCH_INPUT_PIN_0, debounce_timer_callback);
	debounce_timer_1 = xTimerCreate(
		"debounce_timer_1", pdMS_TO_TICKS(DEBOUNCE_DELAY_MS), pdFALSE,
		(void *)GPIO_SWITCH_INPUT_PIN_1, debounce_timer_callback);
	...
	/*
         ...
         */
	gpio_isr_handler_add(GPIO_SWITCH_INPUT_PIN_0, gpio_button_isr_handler,
			     (void *)GPIO_SWITCH_INPUT_PIN_0);
	gpio_isr_handler_add(GPIO_SWITCH_INPUT_PIN_1, gpio_button_isr_handler,
			     (void *)GPIO_SWITCH_INPUT_PIN_1);
